...
void opsinDynamicsImage(float* src[3], float* temp[3], int width, int height, GaussianHandle& gaussianHandle, float intensity_multiplier, hipStream_t stream){
...
void linearRGB(float* src[3], int width, int height, hipStream_t stream){
    int th_x = std::min(256, width*height);
    int bl_x = (width*height-1)/th_x + 1;
