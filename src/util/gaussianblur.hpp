...
__launch_bounds__(256)
__global__ void butterHorizontalBlur_Kernel(float3* src, float3* dst, int w, int h, float* gaussiankernel){
    int x = threadIdx.x + blockIdx.x*blockDim.x;
    int size = w*h;
    if (x >= size) return;
...
void butterGaussianBlur(float3* src, float3* dst, float3* temp, int w, int h, float* gaussiankernel_d, int kerneltotalsize, hipStream_t stream){
    int th_x;
    int bl_x;
    int wh = w*h;
    ...
    th_x = std::min(256, wh);
    bl_x = (wh-1)/th_x + 1;
    butterHorizontalBlur_Kernel<<<dim3(bl_x), dim3(th_x), 0, stream>>>(src, temp, w, h, gaussiankernel_d);
    GPU_CHECK(hipGetLastError());
    butterHorizontalBlur_Kernel<<<dim3(bl_x), dim3(th_x), 0, stream>>>(temp, dst, h, w, gaussiankernel_d);
    GPU_CHECK(hipGetLastError());
}
